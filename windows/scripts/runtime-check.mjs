...
const device=process.argv.includes('--cpu')?'cpu':process.argv.includes('--auto')?'auto':'gpu';
const gpu=process.env.DAVE_TEST_GPU??'auto';
const root=path.resolve(import.meta.dirname,'../..'),app=path.join(root,process.argv.includes('--parity')?'release/windows-parity/DAVE':'release/windows/DAVE');
...
try{const response=new Promise((resolve,reject)=>{lines.on('line',line=>{try{const r=JSON.parse(line);if(r.id==='gpu-check')resolve(r);}catch(e){reject(e);}});child.on('error',reject);child.on('exit',code=>reject(Error(`Backend exited ${code}: ${stderr}`)));});child.stdin.write(JSON.stringify({id:'gpu-check',command:'transcribe',model:'whisper-tiny',device,gpu,path:file})+'\n');const r=await response;assert.equal(r.error,undefined);assert.match(r.result.text,/garden|notebooks|kitchen/i);assert.ok((device==='cpu'||process.argv.includes('--expect-cpu')?['cpu']:['cuda','vulkan']).includes(r.result.acceleration?.provider),JSON.stringify(r));console.log('WINDOWS_RUNTIME_'+device.toUpperCase()+'_OK '+JSON.stringify(r.result));}finally{clearTimeout(timeout);child.stdin.end();child.kill();lines.close();await rm(session,{recursive:true,force:true});}
