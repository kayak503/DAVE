...
const app=resolve('release/native/Local Voice.app');
...
const diagnostic=JSON.parse(run(join(app,'Contents/MacOS/LocalVoice'),['--diagnose']));assert.equal(diagnostic.version,pkg.version);assert(diagnostic.native&&diagnostic.runtime&&diagnostic.backend);
const probe=JSON.parse(run(join(app,'Contents/MacOS/LocalVoice'),['--accessibility-probe']));assert.equal(typeof probe.trusted,'boolean');assert.equal(typeof probe.canPostEvents,'boolean');assert.equal(typeof probe.focusedApplicationError,'number');
run('/bin/bash',['-n','macos/scripts/install.sh']);
const archive=resolve(`release/Local Voice-Native-${pkg.version}-${process.arch}.zip`);
const dmg=resolve(`release/Local Voice-${pkg.version}-macos-${process.arch}.dmg`);
...
try {
 ...
 const unpacked=join(temp,'Local Voice.app');run('codesign',['--verify','--deep','--strict',unpacked]);
 ...
 assert.deepEqual(await readFile(join(unpacked,'Contents/MacOS/LocalVoice')),await readFile(join(app,'Contents/MacOS/LocalVoice')),'Archive must contain current executable');
