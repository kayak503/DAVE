...
test('Missing companion remains CPU automatically and actionable error explicitly',async()=>{const {e}=engine({gpu:false});assert.equal((await run(e,'auto')).acceleration.provider,'cpu');await assert.rejects(run(e,'metal'),/Download GPU files/);});
