...
SOURCE_APP="$SCRIPT_DIR/../../release/native/DAVE.app"
DESTINATION="$HOME/Applications/DAVE.app"
...
if /usr/bin/pgrep -x LocalVoice >/dev/null || /usr/bin/pgrep -x DAVE >/dev/null; then
  echo "Save your work and quit DAVE or the previous Local Voice app before installing." >&2
  exit 1
fi
