...
const output=path.resolve(`release/Local Voice-${version}-macos-${process.arch}.dmg`);
function run(bin,args){const r=spawnSync(bin,args,{stdio:'inherit'});if(r.status!==0)throw Error(`${bin} failed`);}
try{
 run('codesign',['--verify','--deep','--strict',app]);
 run('ditto',[app,path.join(staging,'Local Voice.app')]);
 await symlink('/Applications',path.join(staging,'Applications'));
 run('hdiutil',['create','-volname',`Local Voice ${version}`,'-srcfolder',staging,'-ov','-format','UDZO',output]);
