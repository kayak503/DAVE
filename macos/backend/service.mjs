...
const { SpeechEngine } = await import(pathToFileURL(core));
...
async function dispatch(r) {
  switch (r.command) {
    case 'list': return await engine.listModels();
    case 'install': allowNetwork = true; try { await engine.installModel(r.model); return {}; } finally { allowNetwork = false; }
    case 'remove': await engine.removeModel(r.model); return {};
    case 'synthesize': {
      const result = await engine.synthesize({ modelId: r.model, text: r.text, voice: r.voice, speed: 1 });
      const output = path.join(tempDir, `${randomUUID()}.wav`); await writeFile(output, wav(result.audio, result.sampleRate), { flag: 'wx', mode: 0o600 }); return { path: output };
    }
    case 'transcribe': {
      ...
      const result = await engine.transcribe({ modelId: r.model, device: r.device, audio, sampleRate: 16000 });
    ...
    case 'transcribe-chunk': {
      ...
      if (r.separateSpeakers === true && turns.length) {
        ...
        for (const turn of speakerRegions(turns, audio.length / 16000)) {
          ...
          const result = await engine.transcribe({ modelId: r.model, device: r.device, audio: audio.slice(start, end), sampleRate: 16000, timestamps: true });
      ...
      const result = await engine.transcribe({ modelId: r.model, device: r.device, audio, sampleRate: 16000, timestamps: true });
...
let queue = Promise.resolve(), pending = 0, buffer = '', stopping = false;
...
process.stdin.on('data', chunk => {
  ...
  while ((index = buffer.indexOf('\n')) >= 0) {
    ...
    pending++;
    queue = queue.then(async () => { try { if (stopping) return; send({ id: r.id, result: await dispatch(r) }); } catch (e) { send({ id: r.id, error: e.message || 'Speech operation failed.' }); } finally { pending--; } });
  }
});
function shutdown() {
  if (stopping) return;
