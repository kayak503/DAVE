...
struct LocalVoiceMac: App {
    ...
    var body: some Scene {
        Window("Local Voice", id: "main") { MainView(model: model).frame(minWidth: 760, minHeight: 520) }
        ...
        MenuBarExtra("Local Voice", systemImage: "waveform") {
            Button("Show Local Voice") { NSApp.activate(ignoringOtherApps: true); NSApp.windows.first(where: { $0.canBecomeMain && !($0 is NSPanel) })?.makeKeyAndOrderFront(nil) }
            Divider()
            ...
            Divider()
            SettingsLink()
            Button("Quit Local Voice") { NSApp.terminate(nil) }.keyboardShortcut("q")
        }
    }
}
